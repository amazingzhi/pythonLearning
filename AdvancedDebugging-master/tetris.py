...
from tetris_utils import colors, COLUMN_COUNT, ROW_COUNT, tetris_shapes, MARGIN, WIDTH, SCREEN_HEIGHT, HEIGHT, \
    SCREEN_WIDTH, SCREEN_TITLE
...
def check_collision(board, shape, offset):
    ...
    for cy, row in enumerate(shape):
        for cx, cell in enumerate(row):
            if (new_y := cy + off_y) >= len(board) or \
                    cell and board[new_y][cx + off_x]:
                return True
    return False
